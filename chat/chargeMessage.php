...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    if(isset($_GET['lastId'])){
        ...
        // On écrit la requête
        $sql = 'SELECT `messages`.`id`, `messages`.`message`, `messages`.`created_at`, `USER`.`email` FROM `messages` LEFT JOIN `USER` ON `messages`.`USER_id` = `USER`.`id`'.$filtre.' ORDER BY `messages`.`id` DESC LIMIT 5;';
